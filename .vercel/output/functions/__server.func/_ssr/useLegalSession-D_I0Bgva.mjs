...
//#region node_modules/.nitro/vite/services/ssr/assets/useLegalSession-D_I0Bgva.js
...
var LegalSessionContext = (0, import_react.createContext)(null);
function LegalSessionProvider({ children }) {
	...
	const [chatPending, setChatPending] = (0, import_react.useState)(false);
	const [flaggedIds, setFlaggedIds] = (0, import_react.useState)([]);
	const analyze = (0, import_react.useCallback)(async (content, filename) => {
		setStatus("analyzing");
		setError(null);
		try {
	...
	const toggleFlag = (0, import_react.useCallback)((riskId) => {
		setFlaggedIds((current) => current.includes(riskId) ? current.filter((value) => value !== riskId) : [...current, riskId]);
	}, []);
	const isFlagged = (0, import_react.useCallback)((riskId) => flaggedIds.includes(riskId), [flaggedIds]);
	const askQuestion = (0, import_react.useCallback)(async (question) => {
	...
	const flaggedRisks = (0, import_react.useMemo)(() => (analysis?.risks ?? []).filter((risk) => flaggedIds.includes(risk.id)), [analysis, flaggedIds]);
	const buildPrepSheet = (0, import_react.useCallback)(async () => {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(LegalSessionContext.Provider, {
		value,
		children
	});
}
function useLegalSession() {
	const context = (0, import_react.useContext)(LegalSessionContext);
	if (!context) throw new Error("useLegalSession must be used inside a LegalSessionProvider");
	return context;
}
//#endregion
export { countWords as a, countByCategory as i, CATEGORY_PLURAL as n, formatRelativeTime as o, LegalSessionProvider as r, useLegalSession as s, CATEGORY_LABEL as t };
